#!/bin/bash
# Reaper-proof battery launcher for the #66 one-motion lane (beamr 0.19.1 +
# haematite 0.9.0 + the release bumps). Same shape as
...
#
# GATES, IN ORDER:
#   (a) the battery-queue TOKEN FILE -- entries 1..4 all marked DONE
...
WT=/Users/tom/Developer/ablative/stack/liminal-wt-66-one-motion
GD="$WT/gate-logs/p66-one-motion"
UNITLOG="$GD/unit.log"
...

{
  ...
  echo "GATE_A_WAIT token file, entries 1..4 must all be DONE, at $(ts)"
  while :; do
    if [ ! -r "$TOKEN" ]; then
    ...
    if [ -z "$pending" ]; then
      echo "GATE_A_OK all of 1..4 DONE at $(ts)"
      break
    fi
