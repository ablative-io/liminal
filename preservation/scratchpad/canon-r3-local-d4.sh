...
release_claim() {
  if claim_is_mine; then
    ...
    log "exit without release: claim on file is NOT ours (holder $(claim_field pid < "$CLAIM" 2>>"$CLAIMLOG")) — leaving it in place; RUN IS VOID AS EVIDENCE (A5)"
  else
    # A5 third delta (Cally, 4081cb3d): an ABSENT claim at release is VOIDING, same as
    # foreign. We wrote it, held it, and trapped release on every exit path — if it is
    # gone, someone took it, and the quiet-floor premise is void. A silent no-op here
    # is the unguarded branch that lets the victim publish a green.
    log "RELEASE FOUND NO CLAIM ON FILE — our claim was TAKEN mid-run; quiet-floor premise VOID; RUN IS VOID AS EVIDENCE (A5, 4081cb3d). OPERATOR: post void to the lane, re-run."
  fi
}
