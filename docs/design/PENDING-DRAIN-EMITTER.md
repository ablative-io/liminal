...
| S-16 | **NEW r4** — LIM-DETACHED-PENDING ruling: Detached-flavored candidate drain = faithful detach finalization (§3A.3): committed Detached terminal in the `StoredDetachedCause` domain, end state committed `BindingState::Detached`, slot + enrollment token PRESERVED, participant stays exact-secret resumable, victim becomes a parked `produced` recipient; Died-style identity erasure FORBIDDEN for this flavor; R-A2 "binding-slot release" read as release-from-pending-residence (reading flagged for the tear, §3A.3) | proposed r4 — **TORN 2026-07-23 PASS** (named review, PASS-WITH-FINDINGS, ~18:02Z, no MAJOR; T1 projection-silence ruling folded), r4 tearer-merged at `c41af22`; **LANDED on main at `6d09bae`** — fix `bbb3ace` ("Detached-flavor candidate-lane drain = faithful detach finalization (S-16)"), suites `6f3febb`, clippy `4a5de6a`, gates evidence `98dcf78` GREEN 1785/42; released 0.5.0 (`2a15a23`). **The §3A.3 R-A2 flavor reading — "release" as the slot's transition OUT of pending residence, slot and token preserved — was RATIFIED at that tear** (it was flagged there per stop-and-ask, not silently interpreted), and is now also carried as carve-out 3 of the R18 ratification (main `1a0b60c`). Status amended forward 2026-07-28; row body above unchanged |
| S-17 | **NEW r4** — Detached drain row requirements: distinguishable-by-shape replay, additive durable schema, cause-preserving, drain provenance equivalent to `StoredDrainedTerminal`; exact spelling to the build (§3A.5) | proposed r4 — **TORN 2026-07-23 PASS** (named review, PASS-WITH-FINDINGS, ~18:02Z, no MAJOR; T1 projection-silence ruling folded), r4 tearer-merged at `c41af22`; **LANDED on main at `6d09bae`** — the row shape and additive durable schema built in `bbb3ace`, replay dispatch clippy-cleaned at `4a5de6a`, gates evidence `98dcf78` GREEN 1785/42; released 0.5.0 (`2a15a23`). Status amended forward 2026-07-28; row body above unchanged |
| S-18 | **NEW r4** — Detached extension pins (§3A.7): red-first defect pin, commits-after-drain, slot/token preserved + NOT ParticipantUnknown, victim included-and-parked in recipients, post-drain resume replays parked publication, R-D1 Detached-prestate census, replay-clean idempotence, mixed-flavor ordering, e2e analog (tearer sizes) | proposed r4 — **TORN 2026-07-23 PASS** (named review, PASS-WITH-FINDINGS, ~18:02Z, no MAJOR; T1 projection-silence ruling folded), r4 tearer-merged at `c41af22`; **LANDED on main at `6d09bae`** — pins built red-first in `bbb3ace`, pins 9 + 5 (live drain, live resume with parked replay, unclean restart) in `6f3febb` "test(server): Detached drain e2e", suites rustfmt/clippy-cleaned at `4a5de6a`, red-run log committed at `6d09bae` itself (reviewer-requested completion); gates evidence `98dcf78` GREEN 1785/42 at `4a5de6a`; released 0.5.0 (`2a15a23`). Status amended forward 2026-07-28; row body above unchanged |
| S-19 | **NEW r4** — ASK-6 sequenced OUT of this design and the emitter/Detached builds, with §8A's recorded reasoning, the binding non-interference commitment, and a proposed named board row for its own design-first round | proposed r4 — **TORN 2026-07-23 PASS with r4** (named review, PASS-WITH-FINDINGS, ~18:02Z, no MAJOR), r4 tearer-merged at `c41af22`. Unlike S-16/17/18 there is no build landing to cite — the sequencing-out IS the ruling's whole content: ASK-6 stays out, its own design-first round remains unopened. Status amended forward 2026-07-28 (fold-owner ruling; the sweep flagged this row rather than amend on inference); row body unchanged |
