...
(`13c5cdf:docs/design/WIRING-LEDGER.md:73-103`).
...
### R1.3 census amendment — W7 inherits route 4 by row shape

**Yes: the ObserverRecovery absent-owner pre-pass reaches full
`ConversationOutbox` reconstruction, but it materializes each W7 index only for
the durable row shapes that feed that index.** The pre-pass calls
`replay_and_repair`
...
(`crates/liminal-server/src/server/participant/production/handler.rs:250-268`).
Replay constructs an `ExtensionMerge`, applies every physical extension row,
and installs its `ConversationOutbox` at finish
...
The qualifier is load-bearing. `apply_row` branches by kind: `Produced` alone
feeds `source_batches` and `all_obligations`; `AckAdvanced` alone feeds
`ack_sources`; `MarkerAckCommitted` feeds none of those indexes
(`crates/liminal-server/src/server/participant/production/outbox.rs:168-202,205-270,298-325`).
Thus W7 inherits all four restore routes for corresponding row shapes, and an
all-three-coverage measurement fixture MUST construct both `Produced` and
`AckAdvanced` rows rather than infer coverage from route reachability or
MarkerAck rows. This is the exact r1.3 ledger amendment
(`13c5cdf:docs/design/WIRING-LEDGER.md:79-92`).
...
After pass-2 extension EOF, repair timing remains exactly today's timing. When
the merge reaches a base boundary whose expected tail projection is absent, it
immediately appends and flushes that projection, then applies it to the staged
outbox
(`crates/liminal-server/src/server/participant/production/outbox_replay.rs:97-115`;
`crates/liminal-server/src/server/participant/production/outbox_log.rs:266-284`).
Replay then continues through later base rows, and `finish` runs only after those
rows and the final base-state checks
(`crates/liminal-server/src/server/participant/production/ops_session.rs:324-349`;
`crates/liminal-server/src/server/participant/production/outbox_replay.rs:120-136`).
Consequently a missing-tail repair MAY survive a later base semantic failure
while no authority is published. This durable side-effect ordering is current
behavior and is preserved.

Deferring repair until after `finish` is FORBIDDEN. It would change the durable
bytes and crash cut relative to today; it would also require retaining every
missing expected tail projection until the end, recreating an unbounded
history-sized accumulation that W3 exists to remove.
...
This is a signed correctness improvement over the `DurableStore` abstraction,
not a claim that every generic-store observation is unchanged. Today's
`read_all` is the violating party: the trait promises only up-to-limit pages,
while the reader treats a short page as terminal. Strengthening the trait to
outlaw conforming short-page stores is REJECTED; that would hide the reader bug
...
**W3-D4 — immediate repair placement.** Once pass-2 extension EOF is confirmed,
each missing tail projection is appended/flushed at its base boundary before
later base rows. The repair can survive a later semantic failure. Owner,
observer, capacity, and publication remain unpublished on that failure; durable
repair is not part of their atomic visibility boundary.

`ExtensionMerge::finish` remains the only installation of the fully checked
outbox into the local authority
(`crates/liminal-server/src/server/participant/production/outbox_replay.rs:120-136`).

### 4.3 Authority visibility versus durable repair across all four routes
...
and prevents the recovery batch from entering observer classification. W3 must
not move owner, observer, capacity, delivery, or publication visibility into
either page loop.

That post-finish boundary applies to owner, observer, capacity, and publication
only. It does not roll back durable missing-tail repairs. After pass-2 EOF, a
repair may append/flush before `finish` and may survive a later base semantic
error, exactly as section 3.3 records. Calling repair post-finish-atomic would
be an overreach and is forbidden.
...
### 5.2 Restore-level error sum and both cursor passes

The new restore seam SHALL return a sum that keeps cursor failures distinct from
semantic replay failures in both passes:

- `RestoreError::Extension(OutboxLogError)` owns durable extension cursor
  read/decode/schema/sequence failures from pass 1 **and pass 2**; and
- `RestoreError::Semantic(StateError)` owns base/extension semantic merge,
  projection, authority, and finish failures.

The durability bridge remains the outer transport error. At the production
handler, `Extension` maps through `outbox_log_error`, preserving
`participant Unit 2 extension log failed: {error}`
(`crates/liminal-server/src/server/participant/production/handler.rs:256-258,412-415`).
`Semantic` alone maps through `state_error`, whose wrapper is
`participant production operation failed: {error}`
(`crates/liminal-server/src/server/participant/production/handler.rs:259-268,400-403`).
A pass-2 cursor `?` MUST NOT convert into `StateError` or leak through the generic
semantic wrapper.

A validation-pass extension error returns the same concrete `OutboxLogError` and
production wrapper as today. The application pass never starts, no repair append
occurs, and no owner/observer/capacity prefix exists. A payload containing only
`OUTBOX_SCHEMA_VERSION` still yields
...
Pass 2 re-runs the same cursor checks while applying to unpublished staging. A
pass-2 cursor read/decode/schema/sequence failure returns `Extension`, drops the
staged authority/current page, and cannot repair because extension EOF has not
been confirmed. A semantic or final-boundary failure returns `Semantic` and
drops staged authority; if it occurs after a missing-tail repair, that already
flushed repair survives as section 3.3 requires. W3 adds no resume checkpoint,
partial authority, compatibility decoder, or alternate public error.
...
This cost is not limited to startup. Every source-advancing live commit enters
post-commit `replay_and_repair` before its correlated terminal response and
therefore pays the second complete extension read/decode pass
(`crates/liminal-server/src/server/participant/production/handler.rs:206-223`).

## 7. Acceptance items — complete eleven-oracle census

The nine r2 names remain; items 10 and 11 are first-class re-review oracles.
Every asynchronous proof uses deterministic store/barrier seams, not sleeps or
RSS guesses. Each exact oracle name appears in backticks once; those names are
the tear census.
...
| 5 | `restore_retained_authority_counts_are_measured_and_ledgered` | Restore more than two pages of fully acknowledged/reclaimed history whose fixture explicitly includes `Produced` rows (feeding `source_batches`/`all_obligations`) and `AckAdvanced` rows (feeding `ack_sources`); MarkerAck rows alone do not cover any of the three. Assert the live-record/charge fixture is reclaimed, then use a `cfg(test)` deep-accounting seam to measure and emit a machine-readable count and retained owned bytes separately for all three indexes. The implementation fold records those measurements as W7 evidence. W3 asserts measurement completeness, not an upper bound or constancy claim. |
...
## 8. Shared triggers and main exposure

R1.3 prints the shared trigger verbatim on both rows. W3 says:
...
W7 now says verbatim on its own row:

> HARD, SHARED WITH W3 — before any deployment with unbounded outbox history, BOTH W3 and W7 must be discharged. Stated on both rows so neither landing alone can be read as unblocking unbounded history.

Both quotations are pinned to
`13c5cdf:docs/design/WIRING-LEDGER.md:60-62,94-98`. Landing W3 alone removes a
duplicate transient aggregate but does not make an unbounded-history deployment
safe. Landing W7 alone while `read_all` remains would retain the duplicate full
decoded stream. Both lanes must discharge.
...
### 9.1 Narrowed claim, EOF signing, repair timing, and non-goals
...
- A missing-tail repair may append/flush after pass-2 EOF and before `finish`,
  ...
  owner/observer/capacity/publication are post-finish atomic. Deferring repair
  is forbidden.
- W3 does not change the 64-row signed page size, row encoding/schema, physical
  sequence, merge ordering, live outbox accounting, ack/Leave discharge,
  scheduler, transport, or SDK behavior.
...
- W3 adds no partial authority, resume checkpoint, page-level publication,
  compatibility decoder, or new deployed error precedence.

### 9.2 Cost statement

W3 adds **no idle or resident background cost**: no resident task, timer, poll,
background queue, or idle allocation. It does add live-operation work. Every
source-advancing live commit performs post-commit reconciliation before its
terminal response (`crates/liminal-server/src/server/participant/production/handler.rs:206-223`),
so each such commit pays the second complete extension read/decode pass. Startup,
ordinary cold first touch, and the ObserverRecovery absent-owner pre-pass pay it
as well.

Each pass performs an explicit empty EOF confirmation; conforming short pages
can add read calls in both passes. This is the intentional cost of exact deployed
error precedence plus the signed generic-store EOF correction. Peak duplicate
decoded-page residency is lower, but the history-linear authority remains
allocated. Test accounting is `cfg(test)` only and adds no production atomics or
tags.
...
deployment with unbounded outbox history; its owner is **Hermes**; its verbatim
shared HARD trigger appears in section 8; and its oracle floor is a design-first
bounding brief plus the retained-authority measurement family
(`13c5cdf:docs/design/WIRING-LEDGER.md:73-103`). Index
compaction/reconstruction touches ack and conflict semantics and is not safely
foldable into W3. W7 inherits all four restore routes for corresponding row
shapes: its all-three fixture must include the `Produced` and `AckAdvanced`
classes that actually feed the indexes.

No part of narrowed W3 is deferred: both production passes, all four route
owners, exact error precedence, signed EOF behavior, immediate repair placement,
and the complete eleven-oracle floor land together.
...
- **WALL-RESTORE-ERROR-SUM:** cursor `OutboxLogError` remains distinct from
  semantic `StateError` in both passes and keeps the extension-log wrapper.
- **WALL-EMPTY-READ-EOF:** a short nonempty page never proves EOF; only an empty
  read at the checked successor offset does. This rule and its oracle are one
  signed unit.
- **WALL-ALL-ROUTES:** startup, ordinary cold first touch, post-commit
  ...
  with W7 indexes reconstructed only for corresponding row shapes.
- **WALL-AUTHORITY-VISIBILITY:** owner, observer, capacity, and publication
  remain after validation EOF, application EOF, and finish. Durable repair is
  deliberately excluded from this wall.
- **WALL-IMMEDIATE-REPAIR:** after pass-2 EOF, missing-tail repair appends/flushes
  at its base boundary and may survive a later semantic failure; post-finish
  deferral is forbidden.
