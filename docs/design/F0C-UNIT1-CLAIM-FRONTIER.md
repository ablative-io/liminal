...
| `max_retained_record_rows` | 1,024 | Matches the house's existing signed retention scales — these are DEPLOYMENT values, not in-tree defaults (liminal ships no defaults; `ParticipantConfig` requires every field per `config/types.rs:567@77e4845`): server identity slots 1,024, and the server live-receipt scale that was 1,024 at the pin. Restore rejects-not-truncates (`claim_frontier.rs:3758-3775`), and ack-driven floor advancement bounds spans in normal operation (fixture scale: 32). The 1,024 value is unchanged. |
| `closure_episode_churn_limit` | 1,024 | Orders of magnitude above any plausible binding-change cycles per episode (per-participant deployment scales as at 2026-08-13: 8 live receipts, 256 provenance — raised from 64 on 2026-08-06; liminal ships no defaults, `ParticipantConfig` requires every field per `config/types.rs:567@77e4845`, so these are deployment values, not in-tree constants); inside the corrected `2..=u32::MAX` bound. The 1,024 value is unchanged by the raise. |
...
|---|---|---|---|
| r5 | 2026-08-13 | seat/lane #39 | Rationale correction, values unchanged. Two cells cited per-participant and server receipt scales as though they were in-tree defaults; liminal ships no defaults and every `ParticipantConfig` field is a required deployment value (`config/types.rs:567@77e4845`). Row :707's provenance figure was also stale (64, raised to 256 on 2026-08-06). Both cells now say plainly that these are deployment values as at a stated date. No pinned number in this table moves. |
