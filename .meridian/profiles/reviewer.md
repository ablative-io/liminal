...
- **Run the tests.** Execute `cargo test` and `cargo clippy` to verify the implementation builds and passes.
...
4. **Run tests** — execute `cargo test` to verify tests pass, `cargo clippy -- -D warnings` for lint cleanliness
