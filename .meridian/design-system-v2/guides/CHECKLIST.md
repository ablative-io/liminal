...
Markdown with the rest of the cluster by `scripts/render-cluster.py`.
...
- `done` — Boolean. **Flips to true only when the item is delivered AND
  verified by review.** The dev's claim alone never flips it: the dev
  records its per-C# claim in the brief's `dev` block, and the flip waits
  for the review block to list the C-number as verified. A checklist that
  trusts attestations reports false state with confidence. See
  `guides/REVIEW.md` for what verification means.
