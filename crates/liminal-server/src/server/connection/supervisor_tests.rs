...
/// S4 sibling of [`poisoned_registry_still_reclaims_and_expires`], for the
/// CONNECTION registry and its control queue. Reading a poisoned guard as
/// benign ABSENCE is the silent failure: `active_count` reports zero, so the
/// shutdown drain barrier declares the drain complete while live connections
/// are still tracked; `remove` removes nothing, leaking the record, its fd
/// guard and its §5 admission slot; `is_registered`/`has_control`/`pop_control`
/// all report a connection and its queued controls out of existence. Every one
/// of these is a lifecycle-cleanup or state-report path, so each must RECOVER
/// the guard and use the real data — and say so once, loudly.
// The deliberate panic-while-holding-the-guard IS the poisoning mechanism under
// test; the unwrap can only fail if the fresh lock is somehow already poisoned.
#[allow(clippy::unwrap_used, clippy::panic)]
#[test]
fn poisoned_connection_registry_still_counts_and_removes() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    runtime.register(11, None)?;
    runtime.register(12, None)?;
    runtime.push_control(11, ConnectionControl::NotifyShutdown)?;
    runtime.push_control(12, ConnectionControl::ForceClose)?;

    // Poison BOTH supervisor locks: a panic while holding each guard.
    let poisoner = std::sync::Arc::clone(&runtime);
    let _ = thread::spawn(move || {
        let _guard = poisoner.records.lock().unwrap();
        panic!("deliberately poison the connection registry");
    })
    .join();
    let poisoner = std::sync::Arc::clone(&runtime);
    let _ = thread::spawn(move || {
        let _guard = poisoner.controls.lock().unwrap();
    ...
    let counted = log.capture(|| runtime.active_count());
    assert_eq!(
        counted, 2,
        "a poisoned registry must still report its two live connections"
    );
    let output = log.contents();
    assert!(
        output.contains("ERROR") && output.contains("recovered a poisoned"),
        "the recovery must be loud, got: {output}"
    );

    // Registry membership and the control queue: present, not absent.
    assert!(
        runtime.is_registered(11),
        "a tracked connection must not read as unregistered on a poisoned registry"
    );
    assert!(
        runtime.has_control(11),
        "a queued control must not read as absent on a poisoned control queue"
    );
    assert!(
        runtime.remove_control(12, &ConnectionControl::ForceClose),
        "an unconsumed control must still be retractable on a poisoned control queue"
    );
    assert!(
        matches!(
            runtime.pop_control(11),
            Some(ConnectionControl::NotifyShutdown)
        ),
        "a queued control must still be delivered on a poisoned control queue"
    );

    // Removal actually removes. Read the map's TRUTH directly rather than
    // through a production reader that is itself under test here.
    runtime.finish(11);
    let tracked: Vec<u64> = {
        let records = runtime
            .records
    ...
    assert_eq!(
        tracked,
        vec![12],
        "close must drop the record on a poisoned registry — a skipped removal \
         leaks the record, its fd guard and its §5 admission slot"
    );
    assert_eq!(
        runtime.active_count(),
        1,
        "the surviving connection must still be counted"
    );
    Ok(())
}
