...
//!
//! The residence is minted by the two-part condition at
//! `liminal-protocol/src/lifecycle/operations/binding_terminal.rs:212`:
//! retention is at its cap AND a hard observer still lags the terminal's
//! delivery seq. `max_retained_record_rows = 4` plus enroll/attach/enroll/record
//! puts retention exactly at cap; the peer enrolls and never acks, so it lags.
...
use liminal_protocol::wire::{
    ...
    ParticipantAck, RecordAdmissionAttemptToken, ServerValue,
};
...
-> Result<(), Box<dyn Error>> {
    ...
    let binding = live_binding_state(&residence)?
        .ok_or("victim slot vanished from the live authority")?;
    let BindingState::PendingFinalization(PendingFinalization::Died(pending)) = binding else {
        return Err(format!("live connection loss did not rest at pending Died: {binding:?}").into());
    };
...
/// valid secret hits `ops_attach.rs:215`'s invariant — and the SAME request
/// against the same fixture with the binding still Bound does not.
#[test]
...
-> Result<(), Box<dyn Error>> {
    ...
    assert!(matches!(
        ...
        Some(BindingState::PendingFinalization(PendingFinalization::Died(
            _
        )))
    ));
    ...
    assert!(
        ...
        "re-attach against the live pending-Died residence did not raise the ops_attach.rs:215 invariant; it produced: {pending_report}"
    );
    assert!(
        !control_report.contains(ATTACH_INVARIANT),
        "the Bound control raised the ops_attach.rs:215 invariant too, so the discriminator is not the pending residence; control produced: {control_report}"
    );
    Ok(())
}
...
/// (b, unresolved) Is `:215` an INDEPENDENT wall, or is it masking a
/// co-occurring `RetainedRecordLimit` refusal that would fire anyway?
///
/// NOT SETTLED — and this test records exactly why. The residence exists only
/// because retention is at its cap (`binding_terminal.rs:212` requires
...
-> Result<(), Box<dyn Error>> {
    ...
    assert!(
        ...
        "re-attach against the pending residence did not raise the ops_attach.rs:215 invariant; it produced: {attach_after_ack}"
    );
    assert!(
        !control_attach_after_ack.contains(ATTACH_INVARIANT),
        "the Bound control raised the `:215` invariant too: {control_attach_after_ack}"
    );
    Ok(())
}
...
#[test]
fn post_drain_reenrollment_is_walled_by_retention_not_by_identity()
-> Result<(), Box<dyn Error>> {
    let mut residence = mint_live_pending_died_residence()?;
    let peer_connection = residence.peer_connection;
    let peer_id = residence.peer_id;
