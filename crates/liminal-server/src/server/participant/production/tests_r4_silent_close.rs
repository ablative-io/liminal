//! R4 — the credential-attach path must never answer a client with silence.
//!
//! # The defect
//!
...
//! classify the refusal as terminal, so a never-lose-a-record client retries
//! forever, and every retry that does succeed burns a rationed
//! `ProvenanceParticipant` receipt slot until `ReceiptCapacityExceeded` fires
//! and the estate will not boot.
//!
//! # The seam
//!
...
use liminal_protocol::wire::{
    ...
    CredentialAttachRequest, Generation, ServerValue, StaleAuthority,
};
...
fn attach_against_pending_finalization(
    ...
    let generation = Generation::new(generation_value).ok_or("zero generation in R4 fixture")?;
    dispatch_outcome(
        &fixture.handler,
...
/// sits in `PendingFinalization` is answered with silence.
///
/// The request passes `lookup_credential_attach` as `AuthorizedFresh` (that
/// lookup never inspects binding state when deciding authorization), and then
/// dies in `ops_attach::allocate_attach_mode`, whose `PendingFinalization` arm
/// is a bare `StateError::invariant`. Nothing reaches the wire.
///
/// This test FAILS at `ca66543` and must pass once the refusal is mapped.
#[test]
...
/// `Respond`, so the sibling test's failure is a property of the
/// pending-finalization state and not of this test's plumbing.
///
/// It also pins the LOOKUP-stage precedent that the fix must follow: the
/// refusal travels as an ordinary response value, never as a close.
#[test]
