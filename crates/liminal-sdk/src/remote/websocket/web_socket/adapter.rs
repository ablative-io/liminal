...
impl WebSysWebSocketSocket {
        ...
        match action_for_command(command) {
            Ok(BrowserSocketAction::SendBinary(bytes)) => {
                if let Err(value) = self.socket.send_with_u8_array(&bytes) {
                    let description =
                        format!("browser websocket send failed: {value:?}");
                    dispatch(
                        &self.seam,
                        &self.socket,
                        &self.port,
            ...
            Ok(BrowserSocketAction::Close) | Err(_) => Err(BrowserSocketError::DriverContract {
                description: format!("command_send emitted a non-send command: {command:?}"),
            }),
        }
    }
    ...
    pub fn close(&self) -> Result<(), BrowserSocketError> {
        ...
        match action_for_command(command) {
            ...
            Ok(BrowserSocketAction::SendBinary(_)) | Err(_) => {
                Err(BrowserSocketError::DriverContract {
                    description: format!("command_close emitted a non-close command: {command:?}"),
                })
            }
        }
    }
    ...
    fn drop(&mut self) {
        ...
        if live {
            ...
            if let Err(value) = self.socket.close() {
                self.port.emit(AdapterSignal::Fault(
                    AdapterFault::CommandExecutionFailed {
...
fn classify_message_data(
    data: &wasm_bindgen::JsValue,
) -> (BrowserMessageData, Option<String>) {
    ...
    if data.as_string().is_some() {
        let detail = String::from(
            "peer sent a text message on the binary-only liminal route",
        );
        return (BrowserMessageData::Text, Some(detail));
    }
    if data.dyn_ref::<web_sys::Blob>().is_some() {
        let detail = String::from(
            "browser delivered a Blob despite the pinned arraybuffer binaryType",
        );
...
fn dispatch(
    ...
    let step = match seam.try_borrow_mut() {
        Ok(mut seam) => {
        ...
        Err(_) => {
            // Structurally unreachable on the single browser thread (no
            // foreign code runs under a seam borrow); reported typed.
