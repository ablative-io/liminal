...
fn case_44_dcr_leave_fence_and_typed_cursor_release_suffix() -> TestResult {
    ...
    let fenced = fenced_recovery
        .fenced_attach(
            original_debt,
            Event::fenced_recovery_committed(44, h, prior_epoch, recovered_epoch, h + 1),
            DebtCompletion::clear(),
        )
        .map_err(|state| format!("DCR fenced attach failed: {state:?}"))?;
    ...
    let commit = op_recovery
        .fenced_attach(
            original_debt,
            Event::fenced_recovery_committed(44, h, prior_epoch, recovered_epoch, h + 1),
            DebtCompletion::observer_projection(attached_debt, projection),
        )
        .map_err(|state| format!("nonzero DCR fence failed: {state:?}"))?;
    let authority = commit
        .recovered_binding_fate(Event::binding_fate_observed(44, recovered_epoch, h + 2))
        .map_err(|state| format!("exact recovered fate failed: {state:?}"))?;
