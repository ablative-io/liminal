...
pub use pending_fenced::{
    executable_pending_fenced_attach, executable_pending_fenced_attach_after_ordinary_setup,
};
...
use super::{
    ...
    OrderClaims, OrderDirectOwner, OrderHigh, OrderLedger, PendingFinalizationRestore,
    ...
    RetainedRecordCharge, SealedBindingFateToken, SequenceClaimFrontierRestore, SequenceClaims,
    SequenceDirectOwner, SequenceLedger, SequenceProductRangesRestore, StoredEdge,
    TerminalProductRangeRestore, apply_attach_frontier, commit_attach,
};
use application::{apply_dcr_attach, frontier_owner};
...
/// Pre-commit authority for one real Pending-Died fenced Attached finalizer.
pub struct ExecutablePendingFencedAttach {
    /// Frontier owning exact DCR claim blocks and validated marker authority.
    pub owner: LiveFrontierOwner,
    /// Complete harmless recovery description paired with the private authority.
    pub recovery: DetachedCredentialRecovery,
    /// Member authorized to rotate through the fenced attach.
    pub member: LiveMember<[u8; 32]>,
    /// Exact pending Died binding consumed by the fenced attach.
    pub binding: BindingState,
    /// Matching detach cell.
    pub detach_cell: DetachCell<[u8; 32]>,
    /// Credential secret for the prior generation.
    pub attach_secret: AttachSecret,
    /// Prior dead binding epoch.
    pub prior_binding_epoch: BindingEpoch,
    /// New fenced binding epoch.
    pub recovered_binding_epoch: BindingEpoch,
    /// Delivered marker selected by request and proof.
    pub marker_delivery_seq: u64,
    /// Pending terminal admission order.
    pub terminal_order: u64,
    /// Composed terminal delivery sequence.
    pub terminal_delivery_seq: u64,
    /// Fenced Attached admission order.
    pub attached_order: u64,
    /// Fenced Attached delivery sequence.
    pub attached_seq: u64,
    /// Protocol fenced successor floor.
    pub fenced_resulting_floor: u64,
    /// Exact predecessor closure debt.
