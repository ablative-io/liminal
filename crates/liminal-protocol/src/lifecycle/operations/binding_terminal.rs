...
use crate::lifecycle::{
    ActiveBinding, AdmissionOrder, CommittedBindingTerminalPosition, PendingBindingTerminalPosition,
};
...
impl PreparedBindingTerminal {
    ...
    pub fn admit(self, candidate: BindingTerminalCandidateCharge) -> BindingTerminalAdmission {
        ...
        {
            return BindingTerminalAdmission::Refused(Box::new(BindingTerminalAdmitRefused {
                owner: self.owner,
                error: BindingTerminalAdmitError::CandidateCharge,
            }));
        }
        ...
        if has_capacity {
            return BindingTerminalAdmission::Commit(BindingTerminalCommit {
                owner: self.owner,
        ...
        if self.hard_observer_progress < self.key.delivery_seq() {
            return BindingTerminalAdmission::Pending(BindingTerminalPending {
                owner: self.owner,
        ...
        BindingTerminalAdmission::Refused(Box::new(BindingTerminalAdmitRefused {
            owner: self.owner,
            error: BindingTerminalAdmitError::RetainedRecordLimit,
        }))
    }
}
...
impl BindingTerminalCommit {
    /// Consumes the admission into its owner, committed position, and exact charge.
    #[must_use]
    pub fn into_parts(
        self,
    ) -> (
        ...
        (self.owner, self.position, self.charge)
    }
}
...
impl BindingTerminalPending {
    /// Returns the exact hard-observer baseline that blocked this candidate.
    #[must_use]
    pub const fn blocked_at_observer(&self) -> DeliverySeq {
        self.blocked_at_observer
    }

    /// Consumes the admission into its owner and pending position.
    #[must_use]
...
pub enum BindingTerminalAdmitError {
    ...
    CandidateCharge,
    /// Capacity is exhausted without an observer-blocked pending result.
    RetainedRecordLimit,
}
...
pub enum BindingTerminalAdmission {
    /// The exact candidate may commit at its delivery sequence.
    Commit(BindingTerminalCommit),
    /// Observer progress blocks the candidate without consuming its sequence.
    Pending(BindingTerminalPending),
    /// Candidate validation or non-observer capacity refused unchanged.
