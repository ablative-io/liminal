...
/// the bug.
#[test]
#[ignore = "TEMPORARY target-boundary observation scaffolding (fork ruling 3779580e): removed \
            in the commit immediately following the observation run this scaffold exists for. \
            cargo is fail-fast ACROSS test targets, so this red and the server lib binary \
            RUNNING are mutually exclusive at one tree"]
